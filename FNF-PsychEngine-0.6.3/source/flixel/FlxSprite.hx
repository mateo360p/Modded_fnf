...
class FlxSprite extends FlxObject
{
	...
	/**
	 * Function used to change the, uhh scale. 
	 * X & Y are multipliers.
	 * calls `setGraphicSize()` and `updateHitbox()`.
	 */
	public function setScale(?X:Float = 1, ?Y:Float = 0)
	{
		if (X == null || Y == null) return;
		setGraphicSize(Std.int(X * width), Std.int(Y * width));
		updateHitbox();
	}
