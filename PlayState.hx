...
class PlayState extends MusicBeatState
{
		...
		switch (curStage)
		{
			...
			//var groupTest:FlxGroup = new FlxGroup<>();
				...
				if (!ClientPrefs.lowQuality) {
					...
					skyBackground.setPosition(-520, -210);
					skyBackground.scrollFactor.set(0.1, 0.1);
					skyBackground.scale.set(0.7, 0.7);
					add(skyBackground);
				} else {
					skyBackground = null;
					var skyBG:BGSprite = new BGSprite('phillyStreets/phillySkybox', -520, -210, 0.1, 0.1);
					skyBG.scale.set(0.7, 0.7);
					add(skyBG);
				}
				
				var skyline:BGSprite = new BGSprite('phillyStreets/phillySkyline', -450, -165, 0.3, 0.3);
				add(skyline);
  ...
  */
	...
	function weekendOneWaos()
	{
		//that shit that no one notice but looks epic
		var taim:Float = 0.4;
	...
	function noteMiss(daNote:Note):Void { //You didn't hit the key and let it go offscreen, also used by Hurt Notes
		...
		songMisses++;
		if(Paths.formatToSongPath(SONG.song) != '2hot'){ vocals.volume = 0;
		} else{
			if(daNote.noteType != 'Extra Animation') vocals.volume = 0;
		}
			...
			if(!note.noAnimation) {
				...
				if(note.gfNote)
				{
					...
					if(Paths.formatToSongPath(SONG.song) == '2hot'){
						...
						if(note.noteData == 1){
							if(canShoot = true){
								trace('ya podias disparar xd');//you can shoot, why would you reload again?
							} else {
								trace('recargado');
								var animationThing:String = 'singDOWN-alt';
								canShoot = true;
								boyfriend.playAnim(animationThing, true);
								boyfriend.specialAnim = true;
								weekendOneWaos();
							}
							...
							if (canShoot = true) {
								...
								boyfriend.specialAnim = true;
								FlxG.sound.play(Paths.soundRandom('weekend1/shot', 1, 4));
							} else { //valiste kso
								trace('fue p');
							}
						}
